...
class OfflineStorage {
  ...
  async syncAll(): Promise<{ messages: number; tickets: number }> {
    ...
    try {
      ...
      for (const msg of pendingMessages) {
        try {
          // Use backend API through apiClient
          const response = await apiClient.sendMessage(msg.chat_id, msg.content);
          if (response?.success) {
